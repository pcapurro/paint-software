		...
		_maxValues(maxValues)
{
	if (_titles.size() == 0 || _units.size() == 0 \
		...
		throw std::runtime_error("Error! Invalid DialogValuesBox Parameters (fields number (min: 1)).");

	if (_titles.size() > 2 || _units.size() > 2 \
		|| _minValues.size() > 2 || _maxValues.size() > 2)
		throw std::runtime_error("Error! Invalid DialogValuesBox Parameters (fields number (max: 2)).");

	if (_titles.size() != _units.size() || _units.size() != _minValues.size() \
		|| _minValues.size() != _maxValues.size())
		throw std::runtime_error("Error! Invalid DialogValuesBox Parameters (Parameters vectors size mismatch).");
